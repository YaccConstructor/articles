...
\begin{frame}[fragile]
  ...
  \begin{center}
    \begin{tikzpicture}[node distance=2cm, on grid]
      \node[state]           (q_3)                      {$3$};
      \node[state,initial]   (q_0) [above left=of q_3]  {$0$};
      \node[state,accepting] (q_1) [below left=of q_3]  {$1$};
      \node[state,accepting] (q_2) [above right=of q_3] {$2$};  
...
\begin{frame}[fragile]
  ...
  \begin{itemize}
    ...
    \begin{itemize}
      \item $n \geq 0$
      \item $\forall i: e_i = \delta(q_{i-1}, w_i) = q_i$
...
\begin{frame}[fragile]
  ...
  \begin{rutheorem}[]   
     Цепочка $\omega$ \textbf{распознается} КА $\langle Q, \Sigma, \delta, q_0, F \rangle \iff \exists p \in F : \delta'(q, \omega) = p$
  \end{rutheorem}  
