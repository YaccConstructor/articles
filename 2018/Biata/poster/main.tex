...
\vspace{-0.2cm}
\item We evaluate the proposed approach for 16s rRNA detection. Current accuracy is 90\% for validation set (up to 81000 sequences from NCBI and Green Genes batabases), thus we conclude that our approach is applicable. 
\vspace{-0.2cm}
...
\node[state, 
 ...
 \textbf{Sequences}\\
  \vspace{0.3cm}
Genoms parts of fixed lengs. Current length is 512. Length is variable parameter and can be changed in order to increase quality of solution.
...
Parsing algrithm is based on the algorithm~\cite{gsv}, so the grammar can be extended with conjunctive rules~\cite{Okhotin} for pseudocnots description.%~\cite{zier2013rna}.
...
These matrices can be handled as images or binary vectors by using respective types of neuralnetworks (convolutional or binary) and one of topics for future research is to select better approach. Curently we comress each matrix to integer vector.
};
...
One of interesting question is may this classyfier be used for chimeric sequences filtering.
};
...
We use dense newral network with 14 dense layers.
...
{
\textbf{Vectors}\\
Line-by-line compressed matrix representation: sequence of 32 cells (bits) is compressed to unsigned integer. Top right triangle of matrix is always empty, so can be ignored. May compression to 16 bit integer or byte decrease complexity of neural network and improve result quality?
