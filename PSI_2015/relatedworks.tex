...
state $s_{new}$ from $s$ and the next terminal of the input. The pair $(v, s_{new})$ is called 
\emph{push}, which is added to the global collection $\mathcal{Q}$. The set of $\epsilon$-reductions 
...
either equal to the length of the right-hand side of the production, or one less, if the final 
symbol derives $\epsilon$ (nullable symbols are ignored to reduce memory consumption). 
