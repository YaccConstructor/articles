...
{\vspace{0.3em} \smaller Semyon Grigorev$^1$, Dmitry Kutlenkov$^1$, Polina Lunina$^1$ \\  % Author names
\smaller \it $^1${Saint Petersburg State University, JetBrains Reserach, St. Petersburg, Russia } \\ % Author email addresses
\smaller  {\textbf{E-mail:} semyon.grigorev@jetbrains.com, kutlenkov.dmitri@gmail.com, lunina\_polina@mail.ru}}
...
\headerbox{Solution Overview}{name=solution,span=4,column=0,row=1,below=introduction}{
  ...
  \node[state,
        ...
        align = left,
        double,
        text width = 4.8cm](parser)
  {
\textbf{\hspace{16mm} Parser}\\
...
Matrix-based parsing algorithm~\cite{Azimov:2018:CPQ:3210259.3210264}.
  ...
  \node[state,
        ...
        align = left,
        double,
        text width = 4.8cm](pred)
  {
  \textbf{\hspace{12mm}SS Generator} \\
  Database or tool that provides reference secondary structure for each RNA sequence. \\
  Presently, we use CentroidFold tool~\cite{hamada2009prediction}.
  };

  \node[state,
        right = 1.5cm of parser,
        align = left,
        text width = 4.8cm] (mtrx1)
  {
\textbf{\hspace{8mm} Parsing Matrices}\\
$w$ -- sequence \\ $M_P [i,j] = 1$, iff $w[i,j]$ is derivable from grammar start nonterminal (i.e. folds to a stem).
  ...
  \node[state,
        right = 1.5cm of pred,
        align = left,
        text width = 4.8cm](mtrx2)
  {
  \textbf{\hspace{10mm} Contact Maps}\\
  $w$ -- sequence \\ $M_C [i,j] = 1$, iff there is a contact between nucleotides $w[i]$ and $w[j]$ in secondary structure.
 };

 
   \node[state,
        below right = -0.8cm and 2.2cm of mtrx1,
        align = left,
        double,
        text width = 5.8cm](nn)
  {
  \textbf{\hspace{13mm} Neural Network}\\
...
\headerbox {Results}{name=results, column=0, row=0, span=2, below=solution}{

For experimental research we took sequences from RnaCentral~\cite{rnacentral} database with 70\%:10\%:20\% split and trained models on several datasets with fixed sequences length interval with and without alignment.
...
\item Building a model that predicts secondary structure for sequences of any length.
...
\headerbox {Acknowledgments}{name=ack,column=0,span=2,below=results}{
