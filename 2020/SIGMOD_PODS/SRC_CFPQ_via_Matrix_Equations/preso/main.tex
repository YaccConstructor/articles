...
\begin{itemize}
  ...
  \only<1>{\item \color{red} {$R_A = \{(m, n) \mid m\lambda n \text{ --- path in } D, \lambda \in \mathcal{L}(G_A)\}$}}
  \only<2>{\item $ R_A = \{(m, n) \mid m\lambda n \text{ --- path в } D, \lambda \in \mathcal{L}(G_A)\} $
  \vspace{10pt}
