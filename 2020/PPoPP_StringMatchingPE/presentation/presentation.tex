...
\end{frame}
  ...
  \begin{frame}[fragile] \frametitle{Big Data Processing}
    ...
    \end{verbatim}
%    \pause
    {\tikz[overlay,remember picture]{\draw[draw=red, fill opacity=0.2, line width=0.25mm] ($ (xxx) + (5.15,-0.8)$) rectangle ($  (xxx) + (8.6,-1.3)$);}
    \mycallout<1>[opacity=1]{$ (xxx) + (5.15,-0.8)$}{One filter for many data chunks}
    \tikz[overlay,remember picture]{\draw[draw=red, fill opacity=0.2, line width=0.25mm] ($ (xxx) + (8.9,-0.8)$) rectangle ($  (xxx) + (10.8,-1.3)$);}
    \mycallout<1>[opacity=1]{$ (xxx) + (8.9,-0.8)$}{Many data chunks $\Rightarrow$ many procedure runs}
    }
    \vspace{-1cm}
\begin{center}
...
handleData (data)
{
  ...
  \end{verbatim}
\end{minipage}
{\tikz[overlay,remember picture]{\draw[draw=red, fill opacity=0.2, line width=0.25mm] ($ (xxx) + (0,-2.3)$) rectangle ($  (xxx) + (11,-3.7)$);}}
\end{frame}
