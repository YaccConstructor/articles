...
\end{frame}
  ...
  \begin{frame}[fragile] \frametitle{Big Data Processing}
    ...
    \end{verbatim}
%    \pause
    \onslide<2-3>{\tikz[overlay,remember picture]{\draw[draw=red, fill opacity=0.2, line width=0.25mm] ($ (xxx) + (5.15,-0.8)$) rectangle ($  (xxx) + (8.6,-1.3)$);}
    \mycallout<2-3>[opacity=1]{$ (xxx) + (5.15,-0.8)$}{One filter for many data chunks}
    \tikz[overlay,remember picture]{\draw[draw=red, fill opacity=0.2, line width=0.25mm] ($ (xxx) + (8.9,-0.8)$) rectangle ($  (xxx) + (10.8,-1.3)$);}
    \mycallout<2-3>[opacity=1]{$ (xxx) + (8.9,-0.8)$}{Many data chunks $\Rightarrow$ many procedure runs}
    }
    \vspace{-1cm}
\begin{center}
    \onslide<3>{\texttt{filterParams} is static during one data processing session\\
    \vspace{0.5cm}
