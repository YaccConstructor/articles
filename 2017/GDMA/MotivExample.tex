...
\begin{itemize}
  ...
  \item galleries have a ``magic'' property: you can start from any floor, but by following each gallery you either end up one floor above (edge label is `a' \emph{(consider renaming into 'up' and 'down' --- good idea, but in current article we have no space for SPPF with big boxes fro tokens.)}), or one floor below (edge label is `b'). 
\end{itemize}
...
%\emph{(don't forget that you moved this section after the algorithm, so you need to illustrate the algorithm, not show what the algorithm is supposed to do. This sentence should be rephrased.)}
