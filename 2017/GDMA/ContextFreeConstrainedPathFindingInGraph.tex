...
These algorithms are simple, but impose different restrictions.
...
In case of using such parsing algorithms as GLR and GLL there is no need to transform a grammar to CNF for these algorithms, and it can to improve performance of parsing in some cases.
...
Some other properties of parsing algorithms also may be important: for example, in case of usage directed parsing algorithm (CYK is undirected) may be possible to improve evaluation of queries for a given pair of nodes.
Thus, research of applicability of advanced parsing techniques~\cite{Grune} for graph processing is required. 
...
\item Questions on possibility of advanced parsing techniques application for graph processing, and on possibility of top-down algorithms utilization for  answering
