...
Both CYK and Earley parsing algorithms are bottom-up, CYK is undirected, and Earley-based implementation~\cite{GraphQueryWithEarley} is known to have issues with cycles processing. \emph{(Is it not able to parse them whatsoever? --- It use a fixed (by user) upper bound of paths' length which guarantee parsing termination. So, it can process graphs with cycles, but can not process cycles correctly (in some cases))}. 
