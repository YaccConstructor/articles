...
These algorithms are pretty simple, and graph processing with advanced parsing algorithms~\cite{Grune} is an open qiestion~\cite{Hellings16}.
...
Contribution of rhis paper
\begin{enumerate}
\item Proposed algorithm is based on generalized top-down parsing algorithm---GLL~\cite{scott2010gll}---yes we can use advanced techniques.GLL is topdowt, so yes, we can~\cite{Hellings16}
\item we propose a graph parsing technique which allows one to construct structural representation of query result with respect to the given grammar.
...
\item In evaluation we show that advanced techniques allow to increase performance in comparison with CYK-based implamantation
\end{enumerate}
