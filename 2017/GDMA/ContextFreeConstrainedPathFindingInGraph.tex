...
Other properties of parsing algorithms also affect performance: for example, in~\cite{Hellings16} author assumes that it may be possible to improve evaluation of queries for a given pair of nodes by using top-down directed parsing algorithm.
This assumption interesting for research because CYK is bottom-up and undirected, and Eraley is bottom-up, and Earley-based implementation~\cite{GraphQueryWithEarley} has problems with cycles processing \emph{(we need to rephrase the last bit)}. 
In this work we show that this assumption is true.
Thus, we argue that it is required to research the applicability of advanced parsing techniques~\cite{Grune} for graph processing, that also stated in~\cite{Hellings16} as open question and we provide answer on it. 
...
\item We answer on some questions on advanced parsing techniques applicability for graph processing stated in~\cite{Hellings16}.
