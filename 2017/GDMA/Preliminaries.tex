...
In this case, stack of DPDA is replaced with functions call stack. \emph{(Functions call stack functions as a stack of DPDA (haha))}
...
Parsing may be described as a transition of these pointers from the initial position ($i = 0$, $S \rightarrow \cdot \beta $, where $S$ is start nonterminal) to the final ($i = input.Length$, $s \rightarrow \beta \cdot$). \emph{(Strange terminology. Can we say it can be described as a transition system for initial state to the final? Maybe even list the following options in a usual for transition systems manner?)}
At each step, there are four possible cases in processing of these pointers. 
...
There can be several slots $X \rightarrow \cdot \gamma$ in the second case since the algorithm is nondeterministic, so some strategy to choose one of them for furhter parsing is needed.
...
Each descriptor fully describes parser state, thus instead of immediate processing of all variants, GLL stores all possible branches and process them \empth{sequentially late (what?)}.
...
In order to provide termination and correctness, we should avoid duplication of descriptors, and be able to process GSS nodes in arbitrary order. The following additional sets are used for these purposees.
\begin{itemize}
...
\item $P$---popped nodes. This set allows to process descriptors (and GSS nodes) in arbitrary order. \emph{(allow smb to do smth --- other way this expression cannot be used. Rephrase)}
\end{itemize}
...
Generalized LL builds all such trees and compacts them in a special data structure Shared Packed Parse Forest~\cite{SPPF} \emph{(And when you wrote about GSS, you used small letters. Make this consistent throughout the paper)}, which is described in the following section.
