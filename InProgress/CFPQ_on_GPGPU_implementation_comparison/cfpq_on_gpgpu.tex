...
\title{Evaluation of Matrix-Based Context-Free Path Quering Algorithm}
...
\begin{abstract}
Resantly proposed matrix-based algorithm for context-free path querying (CFPQ) evaluation looks very promising because it offloads the most performance-critical parts to boolean matrices multiplication (BMM). 
By this way it is possible to utilize modern hardware and software to achive highe performance CFPQ evaluation. 
We collect and publish a dataset which includes both real data and syntatica data for worst cases.
We provide a number of implementations of matrix-based CFPQ algorithm which utilize different software and hardware features, such as GPGPUs and high-perfprmance libraries for BMM, and compare performance of them.
