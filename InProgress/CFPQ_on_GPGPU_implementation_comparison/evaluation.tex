...
Time is not presented if time limit is expired, or if no memory enough to allocate all necessary data.
...
As we expect, this case is very hard for sparse metrices based implementations: running time grows too fast.
Also we can see, that grammar size is important.
Both queries specify the same restriction, but grammar for Query 2 contains more rules, and as a result, the running time for big graphs differs more than 2 times.

Finally, we can cocnlude that GPGPU utilization for CFPQ can significantly improve performance, but it should be done more research on abvanced optimization techniqes.
That meens that low-level programming is necessary for high-performance solution.
On the other hand, high-level implementation (\textbf{[GPU\_Py]}) are comparable with other GPGPU-based implementations.
So, it may be a way to find balance between implementation complexity and performance.
Highly optimized existinng libraries can be useful: implementation based on m4ri is faster then reference implementation and other CPU-based implementation.
Moreover it is comparable with some GPGPU-based implementations in come cases.
Sparse matrices utilization should be investigated more.
The main question is can ew create efficient implementattion for sparse boolena matrices multiplication.
