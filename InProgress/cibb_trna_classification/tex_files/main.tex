...
%These features can be extracted by using a parsing algorithm, and processed by an artificial neural network.
...
In this work, we describe a way to improve this approach by using parsing only for network training.
Also, we compare the applicability of networks wich handles two representations of parsing result: vector and bitmap image.
Finally, we compare proposed improvements on tRNA classification tasks .}
