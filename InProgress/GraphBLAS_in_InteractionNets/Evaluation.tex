...
In case of quad-tree based representation reordering also can reduce memory consumption.
...
In evaluation we use both original and reordered matrices except \emph{coAuthorsCiteseer} that used reordered only version because original version cause OOM of Inpla.
...
\begin{figure*}
    ...
    \end{subfigure}
    \caption{Additional scaling analysis}
\end{figure*}
...
Regarding \textbf{performance} we can see, that in all cases (Figures~\ref{fig:coAuthors_execution_time},~\ref{fig:bone_and_usroads_execution_time},~\ref{fig:andrews_execution_time},~\ref{fig:bfs_treferthen_execution_time},~\ref{fig:sssp_treferthen_execution_time},~\ref{fig:bfs_gr_3_30_execution_time},~\ref{fig:sssp_gr_3_30_execution_time}) Interaction Nts based implementations in several orders of magnitude slower (up to $10^5$) then both NetworkX and LAGraph. 
Note that in almost the all cases LAGraph is faster that NetworkX. So for all selected cases linear algebra based approach can demonstrate reasonable performance.
...
It may be cased by manual tuning of parameters aimed on performance on big graphs.
...
For \emph{boneS01} reordered is slower. Note that for these graphs F\# demonstrates no difference in performance between original and reordered matrices. 
For Vine for almost the all cases reordered slower. In same cases reordering does not affect performance significantly. 
...
\end{wrapfigure}
Also we analyze total number of reductions presented in Figure~\ref{fig:total_number_of_interactions}. As expected, Vine performs significantly more reductions because compile code to fixed set of basics agents while Inpla allows user to use custom agents. 
Inpla way can be better to optimize domain-specific computations.
