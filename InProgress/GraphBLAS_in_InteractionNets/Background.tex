...
The goal of the GraphBLAS API is to provide basic building blocks for constructing high-performance graph analysis algorithms.
...
The set of agents is $\Sigma=\{0, S, Add\}$ with arities $Ar(0)=0$, $Ar(S)=1$ and $Ar(Add)=2$ and with the rules shown in Figure~\ref{fig:peano_rules}.
...
\begin{scope}[local bounding box=group1, baseline=(group1.center)]
    \inetcell[draw=darkred](add1){$Add$}[U]
    \inetcell[above=of add1.pal, draw=darkred](zero1){$0$}[D]
    ...
    \inetcell[right=6em of add1, draw=darkred](add2){$Add$}[U]
    \inetcell[above=of add2.pal, draw=darkred](zero3){$0$}[D]
...
\begin{scope}[shift={(group1.east)}, xshift=2.5cm, local bounding box=group2, baseline=(group2.center)]
    \inetcell[draw=darkred](addRed1){$Add$}[U]
    \inetcell[above=of addRed1.pal, draw=darkred](sRed1){$S$}[D]
...
\begin{scope}[shift={(group2.east)}, xshift=2.5cm, local bounding box=group3, baseline=(group3.center)]
    ...
    \inetcell[above=of addRed1.pal, draw=darkred](zeroRed1){$0$}[D]
    \inetcell[below= of addRed1.left pax](sRed2){$S$}[U]
    \inetcell[below= of sRed2.middle pax](zeroRed2){$0$}[U]
...
As shown in Lafont's work~\cite{10.1145/96709.96718}, $\lambda$-calculus can be expressed through this formalism.
Additionally, reductions can be performed independently of each other and it has been proven that the order of reductions doesn't matter. This serves as the primary reason for the natural suitability of Interaction Nets for parallel computations.
...
Inpla is a low-level interaction nets programming language, equipped with a parallel runtime. Programming in Inpla requires defining a custom set of rewriting rules for a chosen set of agents. Computation begins when two agents are connected via their primary ports.
\begin{listing}[H]
...
HVM: fixed set of agents (and respective rewriting rules). Bend.  Acceleration in Cuda. \footnote{Issues, problems, eg with QTree}