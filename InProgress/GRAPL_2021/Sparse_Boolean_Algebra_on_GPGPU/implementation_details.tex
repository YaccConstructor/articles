...
Although general and architectural \cho{specifics} are similar, the internal storage formats and algorithms are different.
...
The core of this library relies on Cuda C/C++ language and API, \cho{what with NVCC compiler allows intermix C++ with Cuda specifics}.
...
This operation is implemented in the classic one pass fashion: it allocates a single merge buffer of size $\textit{nnz(A)} + \textit{nnz(B)})$ before the actual merge of matrices $A$ and $B$, what can negatively affect the memory consumption for large matrices with lots of duplicated non-zero values at the same positions.
