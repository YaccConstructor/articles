...
\cnote{C6}{Biggest saving in the paper. The two bullet lists below (9 items) can be one compact list or a three-line paragraph: $a^-$ with $(a^-)^- = a$; $e^- = (v, u)$; $E^\leftrightarrow = E \cup \{e^- \mid e \in E\}$; $L^\leftrightarrow = L \cup \{a^- \mid a \in L\}$; $\lambda^\leftrightarrow_G(e) = \lambda_G(e) \cup \{a^- \mid a \in \lambda_G(e^-)\}$; $G^\leftrightarrow$. The intermediate objects $E^-$, $L^-$, $\lambda^-_G$ are never used outside this list. About 6 lines.}
...
\end{itemize}
For $X \in \{shortest, simple, trails\}$, respectively, the result of the query is
\begin{align*}
      [[Q]]_{X} = \{ \pi \mid \ & \pi \text{ is a 2-path in } G \text{ from } v_s \text{ that is,} \\
                                & \text{respectively, shortest (to its end vertex),} \\
                                & \text{simple, or a trail, and } \omega_{G^\leftrightarrow}(\pi) \cap R \neq \emptyset \}.
\end{align*}
...
%	\item \underline{\textbf{S}}ingle-\underline{\textbf{s}}ource \underline{\textbf{s}}hortest \underline{\textbf{p}}aths (\textbf{SSSP}):
%	\begin{align*}
%	      [[Q]]_{SSSP} = \{ \pi \mid \ & \pi \text{ is a shortest path to any } v \text{ in } G \text{ from } v_s \\ 
...
%	      \begin{align*}
%            [[Q]]_{SDSP} = \{ \pi \mid \ & \pi \text{ is a shortest path from any } v \text{ in } G \text{ to } v_s  \\ 
%                                         & \text{ and } \omega_{G^\leftrightarrow}(\pi) \cap R \neq \emptyset \}.
%          \end{align*}
%	
%	\item \underline{\textbf{S}}ingle-\underline{\textbf{s}}ource \underline{\textbf{a}}ll \underline{\textbf{s}}imple \underline{\textbf{p}}aths (\textbf{SSASP}):
%	      \begin{align*}
%	            [[Q]]_{SSASP} = \{ \pi \mid \ & \pi \text{ is a 2-simple path in } G \text{ from } v_s \\
%	                                          & \text{ and } \omega_{G^\leftrightarrow}(\pi) \cap R \neq \emptyset \}.
%	      \end{align*}
%	
...
%	      \begin{align*}
%	            [[Q]]_{SDASP} = \{ \pi \mid \ & \pi \text{ is a 2-simple path in } G \text{ to } v_s \\
%	                                          & \text{ and } \omega_{G^\leftrightarrow}(\pi) \cap R \neq \emptyset \}.
%	      \end{align*}
%	
%	\item \underline{\textbf{S}}ingle-\underline{\textbf{s}}ource \underline{\textbf{a}}ll \underline{\textbf{t}}rails (\textbf{SSAT}):
%	      \begin{align*}
%	            [[Q]]_{SSAT} = \{ \pi \mid \ & \pi \text{ is a 2-trail in } G \text{ from } v_s \\
%	                                         & \text{ and } \omega_{G^\leftrightarrow}(\pi) \cap R \neq \emptyset \}.
%	      \end{align*}
%	
%	\item \underline{\textbf{S}}ingle-\underline{\textbf{d}}estination \underline{\textbf{a}}ll \underline{\textbf{t}}rails (\textbf{SDAT}):
%      \begin{align*}
%            [[Q]]_{SDAT} = \{ \pi \mid \ & \pi \text{ is a 2-trail in } G \text{ to } v_s \\
%                                        & \text{ and } \omega_{G^\leftrightarrow}(\pi) \cap R \neq \emptyset \}.
%      \end{align*}
%\end{itemize}
...
Single-destination variants reduce to the single-source ones by reversing the query.
Let $R^- = \{ a_n^- a_{n-1}^- \ldots a_1^- \mid a_1 a_2 \ldots a_n \in R \}$; then a path from $u$ to $v_s$ matching $R$ is, read backwards in $G^\leftrightarrow$, a path from $v_s$ to $u$ matching $R^-$, so $[[\langle G, R, v_s \rangle]]_{SD*}$ is obtained from $[[\langle G, R^-, v_s \rangle]]_{SS*}$ by reversing the resulting paths.
...
\gb{Reviewer D8: the abstract scopes the work to single-source queries while single-destination variants were defined and never evaluated. Kept the definitions and added the reduction remark above. Decide whether to keep the SD definitions or drop them.}

Figure~\ref{fig:semantics-examples} presents examples of single-source query results under different semantics on the same graph: \textbf{a)} \emph{all simple paths}, \textbf{b)} \emph{all trails}, and \textbf{c)} \emph{all shortest paths}.

\cnote{C13}{Layout: Fig.~1 uses the full width for three small graphs. Options: $0.3$ of the text width per panel, or merge Figs.~1 and~2 into one running example (same graph and automaton) and drop the duplicated drawings. About 0.1 page.}
