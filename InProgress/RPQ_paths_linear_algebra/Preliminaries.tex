...
\begin{itemize}
	...
	\rs{is $\lambda_G(e) \cup \lambda_G^-(e)$ defined correctly for both original and inverted edges?}
...
We define a \emph{simple path} as a path without repeated vertices and a \emph{trail} as a path without repeated edges.
Path length is measured as the number of edges.
In this setting, we define the following semantic variants of 2-RPQs considered in this paper.
...
\begin{itemize}
	...
	\item \underline{\textbf{S}}ingle-\underline{\textbf{d}}estination \underline{\textbf{s}}hortest \underline{\textbf{p}}aths (\textbf{SDSP}):
	      $$ [[Q]]_{SDSP} = \{ \pi \mid \pi \text{ is a shortest path to any } v \text{ in } \text{ to } v_s \text{ and } \omega_{G^\leftrightarrow}(\pi) \cap R \neq \emptyset \}. $$
...
\begin{figure}[t]
  ...
  \begin{subfigure}[t]{0.32\textwidth}
    ...
    \caption{All trails (one of the longest paths)}
    %\label{fig:sub4}
  \end{subfigure}
  \begin{subfigure}[t]{0.32\textwidth}
    \centering
