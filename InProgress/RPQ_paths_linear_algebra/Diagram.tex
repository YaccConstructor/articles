...
\begin{tikzpicture}[
  ...
  \node (n_title) [above=0.2cm of Na] {$(N^{\langle \cdot \rangle})^T$};
  ...
  \end{scope}
    ...
    \begin{scope}[
      ...
      \node[vertex] (v4) at (-1.6,0.6) {$v_4$};
      \node[vertex] (v2) at (1.1,1.0) {$v_2$};
      \node[vertex] (v3) at (1.8,-1.15) {$v_3$};
  ...
  \begin{scope}[
      shift={($(a_title.west |- Mres.west)+(1.9cm,-1cm)$)},
      >=Stealth,
      vertex/.style={
          circle,
          draw,
  ...
  \begin{scope}[
      shift={($(a_title.west |- Mres.west)+(1.6cm,-3.6cm)$)},
      >=Stealth,
      shorten >=1pt,
      node distance=15mm,
      on grid,
