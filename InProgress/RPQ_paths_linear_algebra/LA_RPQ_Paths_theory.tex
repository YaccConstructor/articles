...
\begin{algorithm}[h]
	...
	\fbox{
      ...
      \textbf{Input:} Semiring $\mathcal{S} = \langle D_{out}, D_{in_1}, \mathbb{B}, \oplus, \otimes, \mathbf{0} \rangle$ with $D_{in_1} = D_{out}$, initial frontier value $\mathbf{1}_{v_s} \in D_{out}$, index unary operator $I^{\langle \cdot \rangle}$, unary result operator $J$ (the identity unless stated otherwise), 2-NFA $\mathcal{N} = \langle Q, \Sigma, \Delta, \lambda_{\mathcal{N}}, Q_S, Q_F \rangle$, graph $\mathcal{G}=\langle V, E, L, \lambda_{\mathcal{G}} \rangle$, source $v_s \in V$ \\
			\textbf{Output:} Vector $P^F$ of size $1 \times |V|$ over $D_{out}$, depending on the instantiation
			\begin{enumerate}
...
\begin{itemize}
	...
	\item An index unary operator $I^P$ is applied to the frontier after each traversal step to filter or transform path information;
...
The algorithm maintains the BFS structure: on iteration $n$, $M_{qv}$ contains all paths of length $n$ admitted by the chosen semantics from $v_s$ to $v$ that correspond to paths from some $q_s \in Q_S$ to $q$ in the automaton.
...
The upper row of products belongs to the label \texttt{a} and the lower one to \texttt{b}; the two rows are combined with $\oplus$ into the new frontier.
...
Each iteration extends paths consistently in the query automaton and in the input graph; the index unary operator filters out extensions that violate the selected path semantics.
