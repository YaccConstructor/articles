...
\gb{The experiments are to be re-run with different competitors and benchmark suites. Record, for every run: the number of queries completed, timed out, and failed per system and semantics; the number of queries each reported statistic is computed over; the definition and direction of the time-ratio metric (the present ``speedup'' rows are undefined in the text and were misread by an ADBIS reviewer); the size of the answer of every query, since the semantics are enumerating; peak memory; the number of threads each system used; the provenance and the filtering criterion of the query set; which statistic of the repeated runs is reported and its variance; and, per query, whether the answer sets agree with the competitors, which the data-model difference noted in Sect.~\ref{sec:preliminaries} makes necessary. Per-query tables for small query sets are more informative than aggregates.}
