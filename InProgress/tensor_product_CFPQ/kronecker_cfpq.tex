...
In this section, we introduce an algorithm for computation of context-free reachability in some graph $\mathcal{G}$ and RSM $R$.

The idea of the algorithm is based on generalisation of the FSM intersection for a RSM, created from input grammar, and an input graph. Since a graph could be interpreted as some FSM, where edges with labels represent transitions between vertices of the graph, and a RSM is composed from set of FSMs, it is clear to evaluate intersection of such machines using classical algorithm for FSM, represented in~\cite{automata:theory:10.5555/1177300}. 
...
As an example consider the following query, theoretical worst case for CFPQ time complexity, proposed by J.Hellings~\cite{hellings2015querying}: graph $\mathcal{G}$ presented in Figure~\ref{input:graph} and context-free grammar $G=(\Sigma,N,P)$ for a language $\{a^n b^n~|~n \geq 1\}$ where:
