...
\begin{itemize}
    ...
    \item The set of production rules $P$ is presented in the Figure~\ref{ProductionRulesExampleQuery}.
\end{itemize}
...
\begin{itemize}
    ...
    \item The set of terminals $$\Sigma' = \{subClassOf, subClassOf^{-1}, type, type^{-1}\}.$$
    \item The set of production rules $P'$ is presented in the Figure~\ref{ProductionRulesExampleQueryCNF}.
\end{itemize}
...
We run the query on a graph presented in the Figure~\ref{ExampleQueryGraph}.
...
We provide a step-by-step demonstration of the work with the given graph $D$ and grammar $G'$ of the Algorithm~\ref{alg:graphParse}. After the matrix initialization in lines \textbf{6-7} of the Algorithm~\ref{alg:graphParse}, we have a matrix $T_0$ presented in the Figure~\ref{ExampleQueryInitMatrix}.
...
Let $T_i$ be the matrix $T$ obtained after executing the loop in lines \textbf{8-9} of the Algorithm~\ref{alg:graphParse} $i$ times. The calculation of the matrix $T_1$ is shown in the Figure~\ref{ExampleQueryFirstIteration}.
...
The calculation of the transitive closure is completed after $k$ iterations when a fixpoint is reached: $T_{k-1} = T_k$. For the example query, $k = 6$ since $T_6 = T_5$. The remaining iterations of computing the transitive closure are presented in the Figure~\ref{ExampleQueryFinalIterations}.
...
Thus, the result of the Algorithm~\ref{alg:graphParse} for the example query is the matrix $T_5 = T_6$. Now, after constructing the transitive closure, we can construct the context-free relations $R_A$. These relations for each non-terminal of the grammar $G'$ are presented in the Figure~\ref{ExampleQueryCFRelations}.
