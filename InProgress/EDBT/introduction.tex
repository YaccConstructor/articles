...
Therefore the question is whether it is possible to create an algorithm for context-free path query evaluation using the relational or the single-path query semantics which permits to speed up computations with GPGPU by using the matrix operations.

The main contribution of this paper can be summarized as follows:
\begin{itemize}
	\item We show how the context-free path query evaluation using the relational or the single-path query semantics can be reduced to the calculation of the matrix transitive closure.
