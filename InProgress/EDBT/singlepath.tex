...
\begin{proof}(Proof by Induction)
	...
	\textbf{Inductive step}: Assume that the statement of the lemma holds for any $k \leq (p - 1)$ and show that it also holds for $k = p$ where $p \geq 2$. Since $a^{(p)} = a^{(p-1)} \cup (a^{(p-1)} \cdot a^{(p-1)})$ then for any $i, j$ and for any non-terminal $A \in N$, $(A, l_A) \in a^{(p)}_{i,j}$ iff $(A, l_A) \in a^{(p-1)}_{i,j}$ or $(A, l_A) \in (a^{(p-1)} \cdot a^{(p-1)})_{i,j}$.
