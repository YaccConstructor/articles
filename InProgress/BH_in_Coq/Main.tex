...
Namely, let \textit{Tt} and \textit{Vt} be such types, then we can define the types of terminals and nonterminals over \textit{Tt} and \textit{Vt} respectively as presented in~\ref{lst:polymorphic-terminals}.
...
Next, we define a function that evaluates the final state of the automaton if it starts from the state $s$ and receives a word $w$. 
...
\begin{listing}[h]
  ...
  Definition convert_terminal_rule 
    ...
    map (fun s1 => R (V (s1, r, next s1 t)) 
        ...
        list_of_states.
    \end{pyglist}
    \caption{Grammar conversion for the terminal rule}
    \label{lst:verbments1}
\end{listing}
