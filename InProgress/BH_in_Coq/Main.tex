...
Overall goal of this section is to provide step-by-step algorithm of constructing the CNF grammar of the intersection of two languages.
...
Namely, the definition of a grammar, definitions of a derivation in grammar, some auxiliary lemmas about the decidability of properties of grammar/derivation, we also use the theorem that states that there always exists the transformation from context-free grammar to grammar in Chomsky Normal Form (CNF).
...
We inheriting the definition of CFG from~\cite{smolkaHofmann2016}. Rule is defined as a pair of a nonterminal and a list of symbols, and grammar is a list of rules~(\ref{lst:grm}).
...
\begin{align*}
(P := fun tr_non phr => 
\\ final_state next (first3 tr_non) (to_word phr) = third3 r). 
\end{align*}
...
TODO: %Другими словами, можно доказать, что только начиная с ... можно получить  %R (V (from, r, to)) [Ts t] el convert_rules G next -> R r [Ts t] el G. 
