...
However, the proof about existence of transformation to CNF had one major flaw that we needed to fix. One could define a terminal symbol as in inductive type over natural numbers[TODO].
...
(\textbf{TODO}: should we write that we have finite-type for states of DFA? Where?)

First, we consider the case when the DFA does not have any state, that is, the number of states is equal to zero. In this case, we can immediately derive a contradiction. By definition, for any DFA an initial state is known. It means that there is at least one state, which contradicts the fact that the number of states is equal to zero.

(\textbf{TODO}: is it correct citation? )

In addition, it is worth mention, that in the proof ~\cite {beigelproof} cases when in grammar an empty word is derivable or a DFA sets an empty language are discarded as trivial. It is assumed that the proof for this cases one can carry out himself.
In our proof, we do not consider these cases as separate ones. Consideration of these cases is included in the corresponding theorems in general formulation.
...
We say that the automaton accepts a word $w$ being in state $s$ if the function $[final\_state \ s \ w]$ ends in one of the final states. Finally, we say that an automaton accepts a word $w$, if the DFA starts from the initial state and ends in one of the final states.
...
Similarly, we can define functions $s\_accepts$ and $s\_dfa\_language$ for DFA with one final state. Since in this case there is only one final state in order to define function $s\_accepts$ it is enough to check the state in which the automaton stopped with the finite state. Function $s\_dfa\_language$ repeats function $dfa\_language$ except that the function for a DFA with one final state should use $s\_accepts$ instead of $accepts$.
...
Now we can to define a function that converts an ordinary DFA into a set of DFAs with exactly one final state.
...
\begin{theorem}
  Let $dfa$ be an arbitrary DFA and $w$ be a word. Then the fact that $dfa$ accepts $w$ implies that there exists a single-state DFA $s\_dfa$, such that $s\_dfa \in split\_dfa(dfa)$. And vice versa, for any $s\_dfa \in split\_dfa(dfa)$ the fact that $s\_dfa$ accepts a word $w$ implies that $dfa$ also accepts $w$.
\end{theorem}
...
%  der G A w ->
%  (R A w \in G) \/ (exists rhs, R A rhs \in G /\ derf G rhs w).
