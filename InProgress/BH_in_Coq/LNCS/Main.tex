...
Now we should prove the theorem that the function of splitting preserves the language.
...
% $$
% P :=  \lambda \ \textit{tr\_non} \ phr \Rightarrow \textit{final\_state} \textit{\ next \ ($fst_3$ tr\_non) (to\_word \ phr) = $thi_3$ r}.
% $$
%
...
% $$
%  (N \to a) \in \textit{convert\_rules} \ G \to next (\textit{fst}_3 \ (\textit{unVar } N)) \ a = \textit{thi}_3 \ (\textit{unVar} \ N)
% $$
% and
% \begin{align*}
% & (N \to [N_1; N_1]) \in \textit{convert\_rules} \ G  \to \dots \to \\
% & \textit{final\_state} \ \textit{next} \ (fst_3 (\textit{unVar  N})) (to\_word (w_1 ++ w_2)) = thi_3 (\textit{unVar  N})
% \end{align*}
%
...
% \item[a]
% %\noindent\textbf{(a)}
% Consider the case when $w$ is an empty word.
...
%
% %\noindent\textbf{(b)}
% Let's now prove for the case when $w$ is non-empty.
