...
Kronecker product based algorithm for context-free path querying (CFPQ) was recently proposed by Egor Orachev et. al.
	We reduce this algorithm to operation over Boolean matrices and extend it with a mechanism to extract all paths of interest.
	Also, we prove $O(n^3/\log{n})$ time complexity of the proposed algorithm, where $n$ is a number of vertices of the input graph.
	Thus we provide an alternative slightly subcubic algorithm for CFPQ which is based on linear algebra and on a classical graph-theoretic problem (incremental transitive closure), rather than the algorithm proposed by Swarat Chaudhuri.
	Our evaluation shows that the proposed solution is applicable for both RPQ and CFPQ over real-world graphs.
	\end{abstract}
