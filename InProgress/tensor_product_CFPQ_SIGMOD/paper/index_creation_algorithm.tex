...
\begin{theorem}{Sublinear insertion time in incremental transitive closure leads to Subcubic CFPQ.}
Suppose the incremental transitive closure problem where only insertion queries are allowed and the result of each insertion is a set of newly connected pairs. If one can solve this problem in $O(n^{1-\varepsilon})$ time per insertion, then one can solve CFPQ in $O(n^{3-\varepsilon})$.
\end{theorem}