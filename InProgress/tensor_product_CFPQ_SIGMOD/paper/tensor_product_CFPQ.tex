...
\begin{document}
	...
	We reduce this algorithm to operation over Boolean matrices and extends with mechanism to extract all paths of interest. 
	Also we prove $O(n^3/\log{n})$ time complexity of the proposed algorithm, were $n$ is a number of vertices of the input graph. 
	%We do it by reducing the algorithm to special case of incremental transitive closure of directed graph.
	Thus we provide an alternative way to construct slightly subcubic algorithm for CFPQ which is based on linear algebra and on classical graph-theoretic problem (incremental transitive closure), rather than way proposed by Swarat Chaudhuri. 
	Our evaluation shows that this algorithm is good candidate to be an universal algorithm for both regular and context-free path querying. 
	\end{abstract}
