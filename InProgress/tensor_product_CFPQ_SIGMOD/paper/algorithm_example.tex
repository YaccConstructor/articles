...
At the last iteration, no more edges will be added to the graph $\mathcal{G}$. However, the new edge $(1, 1) \rightarrow (2,1)$ will be added to the resulting Kronecker product, and the transitive closure
...
Lazy evaluation is required here, since the resulting graph may possibly have an
infinite number of paths between some vertices. Another approach here is
to try to query some fixed number of paths, query paths with fixed maximum length, or just query a single path. Eventually,
