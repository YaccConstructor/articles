...
For example, one can specify regular expression $R_1 = ab^*$ to find paths in the graph $\mathcal{G}$ (fig.~\ref{fig:example_input_graph}). Expected result is set of paths which start with $a$-labeled edge and contain zero o more $b$-labeled edges after that.
...
\subsection{Graph Kronecker Product and Automata Intersection}
...
It is worth mention, that Kronecker product produces blocked matrix $C$, with total number of the blocks $m_1 * n_1$ , where each block has size $m_2 * n_2$ and is defined as $A[i,j] \cdot B$ (scalar to matrix).
...
The above definition of the Kronecker product for directed graphs can be easily described as the Kronecker product of the corresponding adjacency matrices of graphs, which gives us the following definition:
...
Accordingly to~\cite{automata:theory:10.5555/1177300}, the above definition of the FSM intersection allows to construct the new machine with the following property: $L(T) = L(T_1) \cap L(T_2)$. Since a directed graph could be interpreted as an FSM, and an RSM is composed as set of FSMs, one could employ the idea of the Def.~\ref{def:fsm:intersection} to intersect the RSM and the graph.

Using adjacency matrices decomposition for FSMs we can reduce the intersection to the Kronecker product of such matrices over Boolean semiring at some extent, since the transition function $\delta$ of the machine $T$ in matrix form is exactly the same as the product result. More precisely:

\begin{definition}
\label{def:fsm:product}
Given two FSMs $T_1$ and $T_2$ with matrices $\mathcal{M}_1$ and $\mathcal{M}_2$. The intersection machine $T$ with matrix $\mathcal{M}$ defined as follows:
\begin{itemize}
...
As mentioned earlier, an RSM is composed as set of FSMs. From matrix point of the view, there is no matter to use as an input adjacency matrix $\mathcal{M}$ for the FSM or the RSM, since it is transitions matrix, with only difference in the alphabet of the symbols, where $\Sigma$ is used for FSM and $\Sigma \cup N$ is used for the RSM respectively. Thus, the given Def.~\ref{def:fsm:product} is valid for RSM and FSM intersection.

% Example

In this work we show how to express RSM and FSM intersection in terms of Kronecker product and transitive closure over Boolean semiring.
