     	...
     	\For{$(path, q) \in supposedPaths$}
     		...
     		\For{$j \text{ such that } q[j] = 1$}
     			...
     			\If{$path \text{ is empty path}$}
     				\State{$pathNew \gets pathNew \cdot [((s_i, v_i), (j / dim(\mathcal{M}_2), j \bmod dim(\mathcal{M}_2)))]$}
     				%\Comment{Edge adding}
     			\Else
     				\State{$(s_k, v_k) \gets \text{ the last vertex of } path$}
     				\State{$pathNew \gets pathNew \cdot [((s_k, v_k), (j / dim(\mathcal{M}_2), j \bmod dim(\mathcal{M}_2)))]$}
     				%\Comment{Edge adding}
     			\EndIf
