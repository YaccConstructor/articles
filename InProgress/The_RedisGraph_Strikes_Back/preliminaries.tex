...
\begin{align*}
&\mathcal{E}^a = \{(1, 2), (2, 3)\}, &&\mathcal{E}^b = \{(2, 3), (2, 6)\},  \\
&\mathcal{E}^c = \{(3, 5), (5, 4), (4, 3)\}, &&\mathcal{E}^d = \{(5, 6), (6, 5)\}.
...
\begin{align*}
&\mathcal{V}^x = \{(1,1), (3, 3)\}, &&\mathcal{V}^y = \{(5, 5)\}.
\label{eq:boolean_decomposition_of_graph}
\end{align*}
...
$$A \cdot B = \{a b \mid a \in A, b \in B\} \in \Sigma^*$$
...
\end{split}
\end{equation*}
where $l(\pi) \in (\Sigma_V \cup \Sigma_E)^*$ is the set of possible labels along the path $\pi$:
$$l(\pi) = \lambda_V(v_1)^* \cdot \lambda_E(e_1) \cdot \lambda_V(v_2)^* \cdot \lambda_E(e_2) \cdot ... \cdot \lambda_E(e_n) \cdot \lambda_V(v_n)^*$$

\textbf{TODO}: Example of $l(\pi)$ and context free relation.
