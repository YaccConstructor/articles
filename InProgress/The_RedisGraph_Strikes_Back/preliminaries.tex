...
For example adjacency matrix $M$ of the graph $D_1$ is
...
For example, vertex label matrix $H$ of the example graph $D_1$ can be decomposed into a set of the following boolean matrices:
...
\begin{figure}[h]
    ...
    \begin{tikzpicture}[shorten >=1pt,auto]
       ...
       \node[state] (q_5) [below right=of q_1]   {$6$};
       \path[->]
        (q_0) edge[loop above]  node {$\{x\}$} (q_0)
        (q_0) edge[loop left]  node {$\{y\}$} (q_0)
        (q_0) edge  node {$\{a\}$} (q_1)
        ...
        (q_4) edge  node {$\{c\}$} (q_3)
        (q_2) edge[above]  node {$\{c\}$} (q_4)
        (q_4) edge[loop above]  node {$\{y\}$} (q_4)
        (q_5) edge[bend left, below]  node {$\{d\}$} (q_4)
        (q_4) edge[bend left, above]  node {$\{d\}$} (q_5);
    \end{tikzpicture}
...
\begin{definition} \emph{Context-free language} is a language generated by a context-free grammar $\mathcal{G}$:
\begin{align*}
...
Let's consider the example of the context-free grammar $G_1=(N, \Sigma, P, S), ~N=\{S\},~\Sigma=\{c, d, y\}$ given by the two production rules: 
\begin{align*}
S \rightarrow cSd\\
S \rightarrow cyd
\end{align*}
This grammar generates the context-free language $$L(G_1) = \{c^nyd^n, n \in \mathbb{N}\}$$ and after transformation to weak Chomsky normal form the resulting grammar:
\begin{gather*}
...
Modified context-free grammar with these productions generate the same language as the original grammar.
...
\begin{definition}
Let $\mathcal{D} = (V, E, \Sigma_V, \Sigma_E, \lambda_V, \lambda_E)$ be a labeled graph, $\mathcal{G} = (N, \Sigma_V \cup \Sigma_E, P, S)$ be a context free grammar. Then a \emph{context free relation} with grammar $\mathcal{G}$ on the labeled graph $\mathcal{D}$ is the following relation $R_{\mathcal{G}, \mathcal{D}} \subseteq V \times V$:

\begin{equation*} \label{eq1}
\begin{split}
R_{\mathcal{G}, \mathcal{D}} = \{&(v, to) \in V \times V \mid \exists \pi = (v_1, e_1, v_2, e_2, ..., e_n, v_n) \in \pi(D): \\
      &v_1 = v, v_n = to,~l(\pi) \cap L(\mathcal{G}) \neq \varnothing \},
\end{split}
\end{equation*}
...
For example, in the labeled graph presented in figure~\ref{fig:example_input_graph} we have the path $\pi$ from the vertex 3 to vertex 6. The labels along this path can form the sequence $cyd$. It can be observed that this sequence satisfies context-free constraints of the above grammar $G_1$:
    \begin{align*}
...
Finally, in these notations we are ready to define context-free path querying problem.
\begin{definition}
    \emph{Context-free path querying problem} is the problem of finding context-free relations $R_{\mathcal{G}, \mathcal{D}}$ for a graph $\mathcal{D}$ and a context-free grammar $\mathcal{G}$.
    ...
    For above example of graph $D_1$ and context-free free grammar $G_1$ the resulting set $R_{G_1, D_1} = \{(3, 5), (5, 5), (5,6)\}$ and the resulting matrix
\begin{align*}
\mathcal{M} =
\begin{pmatrix}
    . & . & . & . & . & .   \\
...
Let $\mathcal{G} = (N, \Sigma, P)$ be the input grammar, the input edge-labeled graph $\mathcal{D} = (V, E, \Sigma_V, \Sigma_E, \lambda_V, \lambda_E)$ and language $L$ over alphabet $\Sigma$. For the context-free path query evaluation, we need to provide context-free relations \mbox{$R_A \subseteq V \times V$} for every \mbox{$A \in N$}.
...
\label{alg:algo0}
\Function{evalCFPQ}{$\mathcal{D}=(V,E), \mathcal{G}=(N,\Sigma,P)$}
    \State{$n \gets$ |V|}
