...
\begin{definition}
Let $\mathcal{D} = (V, E, \Sigma_V, \Sigma_E, \lambda_V, \lambda_E)$ be a labeled graph, $G = (N, \Sigma_V \cup \Sigma_E, P, S)$ be a context free grammar. Then a \emph{context free relation} with grammar $G$ on the labeled graph $\mathcal{D}$ is the following relation $R_{G, \mathcal{D}} \subseteq V \times V$:

\begin{equation*} \label{eq1}
\begin{split}
R_{G, \mathcal{D}} = \{&(v, to) \in V \times V \mid \exists \pi = (v_1, e_1, v_2, e_2, ..., e_n, v_n) \in \pi(D): \\
...
\begin{definition}
    \emph{Context-free path querying problem} is the problem of finding context-free relations $R_{G, \mathcal{D}}$ for a graph $\mathcal{D}$ and a context-free grammar $G$.
...
Let $G = (N, \Sigma, P)$ be the input grammar, the input edge-labeled graph $\mathcal{D} = (V, E, \Sigma_V, \Sigma_E, \lambda_V, \lambda_E)$ and language $L$ over alphabet $\Sigma$. For the context-free path query evaluation, we need to provide context-free relations \mbox{$R_A \subseteq V \times V$} for every \mbox{$A \in N$}.
...
\label{alg:algo0}
\Function{evalCFPQ}{$\mathcal{D}=(V,E), G=(N,\Sigma,P)$}
    \State{$n \gets$ |V|}
