...
\cho{In other worlds concatenation of two sets contains all concatenations of elements from the first set with all elements from the second one.}
...
\begin{definition} A context-free grammar $G = (N, \Sigma, P, S)$ is in \emph{Chomsky normal form} if every production in $P$ has one of the following forms:
    \begin{itemize}
        ...
        \item $S \rightarrow \varepsilon$, where $\varepsilon$ is an empty string \cho{(or identity element of $\Sigma^*$)}.
    \end{itemize}\qed
\end{definition}
...
The matrix-based algorithm for CFPQ can be expressed in terms of operations over Boolean matrices as presented in listing~\ref{alg:algo0}.
