...
The \texttt{MATCH} clause consists of relation pattern \texttt{[:a | :c]} and path pattern \texttt{/:b $\sim$S/}, and this path pattern uses reference to named pattern \texttt{S}. 
Thus, the specified constraint which says that path of interest should starts in the vertex with label \texttt{x}, than it should goes throw edge with label \texttt{a} or \texttt{c}, and the end of path is a sequence of edges which starts from \texttt{b} and tail of this sequence matches with \texttt{S}.  

For the example graph $D_1$ this query returns the next pairs of vertices \texttt{(v, to)} (as specified in \texttt{RETURN} clause): $\{(0, 4), (0, 5)\}$.
...
While syntax extension proposal is fully implemented in parser, only part of new syntax is supported in RedisGraph. 
As far as the most important part for our work is ability to specify labels of vertices and types of edges, and this part was supported.
Full support of new syntax requires huge effort and should be done in the future.

Important feature of RedisGraph is lazy query evaluation.
Assume that when the RedisGraph processes a query it finds all paths that match the constraints specified in this query. This can be achieved in two ways---one can find all the paths completely at once, or process them gradually one by one. The first method is potentially faster than the second, but requires much more memory.
Moreover, in the first case the size of the consumed memory directly depends on the size of the graph, which can be very large.
In contrast, the second way may be slower, but it consumes much less memory, which is almost independent of the graph size and is usually limited.
This way is called lazy query evaluation and is used in RedisGraph.

To improve performance, the RedisGraph processes paths not just one by one, but groups them into \textit{chunks} of some size.
Thus, we can assume that in one unit of time RedisGraph processes a set of paths with the size of a chunk.
This allows one to achieve higher performance in terms of query processing time.
The chunk size parameter allows developers to adjust the ratio of time and memory consumption---increasing this parameter reduces the query execution time but increases the memory consumption.

The multiple source algorithm fits perfectly into the lazy evaluation of the query, since it allows one to process paths from a given set of vertices.
Thus integrating our algorithm into the RedisGraph is natural and preserves the important property of lazy evaluation.
However, since queries with context-free constraints may require much more execution time than the original Cypher queries, the operation that executes multiple source algorithm must have its own chunk size parameter.
This parameter directly affects the size of the set of source vertices on which a single run of the multiple source algorithm will be performed. For convenience, we have added the ability to change this parameter in the configuration file so that everyone can choose the appropriate parameter for a specific query.
...
Graphs are loaded into RedisGraph database such that each vertex has a property \verb|id| which value is unique and is in $[0 \ldots |V|-1]$, where $|V|$ is a number of vertices in the graph to load.
...
Here \texttt{\{id\_from\}} and \texttt{\{id\_to\}} are placeholders for lower and upper bounds for \verb|id|. The example of the exact query for start set of size 16 is presented in listing~\ref{lst:query_g1}.
...
Queries generator for all three queries ($g_1$, $g_2$, and $geo$) was implemented and used to create queries for all start sets which are used in the previous experiment. 
...
Time and memory consumption are measured for each start set processing.
...
Processing time for all sets, except set of size 10~000 for \textit{geospecies} graph (fig.~\ref{fig:redis_geospecies_all}) is less then 1 second.
Moreover, for sets of size 16 processing median time is less then 0.1 second, except \textit{geospecies} graph.
...
We can see, that amount of used memory depends on graph and query, but for relatively small sets ($\leq 1000$) RedisGraph uses less that 50Mb of RAM to process one set.
...
As a result, we can conclude that multiple-source CFPQ is significantly more memory efficient than creation of full reachability index and its filtering: processing the set of size 10~000 on \textit{geospecies} graph requires less than 200Mb, while full index creation requires 16Gb~\cite{10.1145/3398682.3399163}.  
...
Here chunk size is a chunk size parameter of RedisGraph.
%, and here multiple source algorithm will be evaluated $\frac{|V|}{chunk\_size}$ times. 
%Notice, that not always chunk\_size equal to size of set of source vertices of multiple source algorithm, but for these queries it is true}).
