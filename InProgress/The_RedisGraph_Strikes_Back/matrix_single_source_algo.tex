...
$Src$}
    ...
    \State \simpleton{$MSrc \gets TSrc^S$}
    ...
    \While{$T\ or\ TSrc\ is\ changing$} \Comment{Algorithm's body}
        ...
        \EndFor
    \EndWhile
    \State \Return \simpleton{$MSrc * $}$T^S$
\EndFunction
...
After every iteration of the while loop \simpleton{(in the end of every iteration of for loop)}, it is necessary to update the set of \simpleton{vertices (source vertices?)} paths from which we need to calculate.
To do it, the we call the function \textsc{getDst} (see lines \textbf{17-21}), in line \textbf{14} \simpleton{(in line 15, bit in line 14 source vertices is updated too, but without getdst)}.
Thus, the modified algorithm supports the frontier of the vertices of interest and updates it on each iteration.
As a result, it only computes the paths which starts from the small set of selected vertices \simpleton{(or maybe all paths, f.e. worstcase :D)}.
...
\hskip\algorithmicindent $Index = (T, TSrc)$}
    ...
    \State \simpleton{$MSrc \gets TNewSrc^S$}
