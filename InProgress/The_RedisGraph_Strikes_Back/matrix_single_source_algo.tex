...
\Function{MultiSrcCFPQ}{$D = (V, E, \Sigma_V, \Sigma_E, \lambda_V, \lambda_E),~G=(N,\Sigma,P,S), Src$}
    ...
    \State{$TSrc \gets \{TSrc^A \mid  A \in N \setminus S, TSrc^A \gets \varnothing\}$}
...
As a result, we can conclude that caching is not useful for multiple-source CFPQ for evaluated cases and the na\"{i}ve version of the algorithm is better for implementation in real-world graph database. 
