...
\Function{MultiSrcCFPQ}{$D = (V, E, \Sigma_V, \Sigma_E, \lambda_V, \lambda_E),~G=(N,\Sigma,P,S), Src$}
    ...
    \Comment{Set of matrices, in which every maxtrix is $\varnothing$}
    
    \State{$TSrc \gets \{TSrc^A \mid  A \in N, TSrc^A \gets \varnothing\}$}
    \Comment{Set of matrices for input vertices in which every matrix is $\varnothing$}
...
Lets consider the example of running this algorithm on graph $D_1'$ and grammar $G_1$ in weak Chomsky normal form with set of start vertices $Src = \{2\}$. After input matrix initialization $TSrc^S = \{(2, 2)\}$. Then algorithm initializes set $T$ with simple rules $C \xrightarrow{} c, D \xrightarrow{} d, Y \xrightarrow{} y$ and we get the following matrices:
\begin{align*}
    T^C &= \mathcal{E}^c = \{(2, 4), (4, 3), (3, 2)\} \\
    T^D &= \mathcal{E}^d = \{(4, 5), (5, 4)\} \\
    T^Y &= \mathcal{V}^y = \{(0, 0), (4, 4)\}
\end{align*}

Lets consider the first iteration of the while loop in line~\textbf{9}. The first grammar rule is $S \xrightarrow{} C E$. Here the matrix $M$ is computed to $TSrc^S*T^C = \{(2, 4)\}$. Since matrix $T^E$ is empty, $T^S$ is not updated in line~\textbf{12}. But the input matrix $TSrc^E$ is populated with new records and becomes equal to $\{(4, 4)\}$. This means that we are interested in paths that start from the 4th vertex and satisfy the constraints specified by nonterminal $E$. The second grammar rule $S \xrightarrow{} C S_1$ is very similar to previous one. After processing it, the matrix $T^{S_1}$ also becomes equal to $\{(4, 4)\}$.
Now let's look at the processing of the third rule $E \xrightarrow{} Y D$. Here $M$ is computed to $TSrc^E * T^Y = \{(4, 4)\}$. After that algorithm update $T^E$ with matrix $M  * T^D = \{(4, 5)\}$ and now we know that path $(4, (4, 5), 5)$ satisfies the constraints of $E$. It remains to consider the last rule $S_1 \xrightarrow{} S D$. During it processing since $T^S$ is empty only $TSrc^S$ is updated and becomes equal to $\{(2, 2), (4, 4)\}$. This is the end of the first iteration.

In order to consider at least some path satisfying S, lets look at processing of rule $S \xrightarrow{} C E$ in second iteration. Now matrix $M$ is computed to $TSrc^S * T^C = \{(2, 4), (4, 3)\}$. After first operation $T^E$ was populated and became equal to $\{(4, 5)\}$. So $T^S$ is computed to $M * T^C = \{(2, 5)\}$. Thus, we found the first path that satisfies the grammar constraints. 
