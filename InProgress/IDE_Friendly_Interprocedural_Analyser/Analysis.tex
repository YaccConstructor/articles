...
Then restrictions on paths define sequences of operations leading to an issue.
...
Each edge contains an operation that represents a statement in the source code and at the same time, the target of the edge indicates the position to jump after the execution of the operation.
...
\end{split}
\end{equation}
Where $o$ is an object or a class which method is called, $m$ is the name of a method, $v$ is a variable where the result is stored and $f_i, a_i$ are pairs of the formal and the actual parameter respectively.

Each assignment operation represents the related operation in the source code and is written in the following way:
\begin{equation}
\begin{split}
	& \text{assign:} \\
	& s \rightarrow t
...
Return statement just indicates the end of a method. 
It is necessary to add this statement even it does not exist in source code to inform the analyzer about the return point.
...
So, we propose to resolve invocations dynamically during analysis.
