...
%However, there is another intermediate analysis which is run before any other one to collect some information required by the resolver.
%In particular, it tracks the propagation of all variables to discover all possible concrete types of each variable.
%So, it involves each variable and each method in the whole program and thus the time and space required for the execution of this analysis may be the consistent estimation of the efficiency of the solution.
%
%The codebase which has been chosen as a source of data is the full solution of the Mono project.
...
So, our solution is able to process real-world projects in acceptable time.
