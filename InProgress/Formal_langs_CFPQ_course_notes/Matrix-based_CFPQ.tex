...
\Function{contextFreePathQuerying}{G, D}
    
    \State{$n \gets$ количество узлов в $G$}
    \State{$E \gets$ направленные ребра в $G$}
    \State{$P \gets$ набор продукций из $D$}
...
Пусть $T_i$ --- матрица, полученная из $T$ после применения цикла на строчках \textbf{8-9} алгоритма~\ref{alg:graphParse} $i$ раз. Далее показано получение матрицы $T_1$.
