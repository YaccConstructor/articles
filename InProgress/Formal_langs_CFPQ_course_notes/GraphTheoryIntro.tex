...
Список интересных работ по APSP для ориентированных графов с вещественными весами:
\begin{itemize}
    \item Классический алгоритм Флойда-Уоршелла \cite{Floyd1962, Bernard1959, Warshall1962}. Сложность: $O(n^3)$, где $n$ - количество вершин в графе
    \item M.L. Fredman (1976) --- уже с субкубичной сложностью $O(n^3(\log \log n / \log n)^\frac{1}{3})$ \cite{FredmanAPSP1976}
    \item W. Dobosiewicz (1990) --- $O(n^3 / \sqrt{\log n})$ \cite{Dobosiewicz1990}
    \item T. Takaoka (1992) --- $O(n^3 \sqrt{\log \log n / \log n})$ \cite{Takaoka1992}
...
Пусть даны матрицы $A$ и $B$ размера $n \times n$. Определим операцию $\star$, которую называют \textit{Min-plus matrix multiplication}:
    ...
    $C = A \star B$ --- матрица размера $n \times n$, т.ч. 
...
Данный алгоритм называется \textit{repeated squaring}\footnote{\href{http://users.cecs.anu.edu.au/~Alistair.Rendell/Teaching/apac_comp3600/module4/all_pairs_shortest_paths.xhtml}{Ссылка на решение APSP с помощью repeated squaring}}.

\begin{remark}
Также существует алгоритм Зейделя (Seidel's algorithm)\cite{Seidel1995}, решающий APSP с помощью произведения матриц, но он применим только для неориентированных графов без весов\footnote{\href{http://math.mit.edu/~rothvoss/18.304.1PM/Presentations/2-Chandler-slideslect2.pdf}{Пример для алгоритма Зейделя}}. Его сложность: $\log n * K(n)$, где $n$ - количество вершин, а $K(n)$ - сложность алгоритма умножения матриц.
Несмотря на то, что алгоритм имеет хорошую теоретическую сложность, на практике он не эффективен\cite{Lavallee2004}(одной из причин является нетерминирующая рекурсия).

\end{remark}

\subsection{Субкубичность}
...
Заметим, что булевы матрицы не имеют обратный элемент относительно операции дизъюнкции (т.е. для произвольной булевой матрицы $A$: $\nexists D$, т.ч. $D$ --- булева матрица и $A \vee D = D \vee A = N$), поэтому они не являются кольцом.
