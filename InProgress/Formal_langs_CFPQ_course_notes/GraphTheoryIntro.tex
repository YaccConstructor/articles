...
$C = A \star B$ --- размера $n \times n$, т.ч. 
...
\Function{Seidel}{$\mathcal{G}$}
    \State{$A \gets$ матрица смежности $\mathcal{G}$}
    \State{$Z \gets A \times A$}
    \State{$B \gets$ матрица, состоящая из 0 и 1 }
    \begin{displaymath}
        B[i,j] =
            \left \{
            \begin{array}{ll}
    ...
    \If{$ \forall i \neq j$ $B[i,j] = 1$}
        \State{\Return $D = 2B - A$}
    \EndIf
    \State{$T \gets Seidel(B)$}
    \State{$X \gets T \times A$}
    \State{$D \gets$ матрица с расстояниями, т.ч.
        \begin{displaymath}
        D[i,j] =
            \left \{
            \begin{array}{ll}
                2T[i,j] & \textrm{, если $X[i,j] \geq T[i,j] * degree(j)$,}
                \\
                2T[i,j] - 1 & \textrm{, иначе} 
            \end{array} \right.
        \end{displaymath}
    }
    \State \Return $D$
...
\begin{example}[APSP с помощью алгоритма Зейделя]
  Пусть дан следующий граф:
  
  \begin{center}
  \begin{tikzpicture}[on grid,auto]
     \node[state] (q_0)   {$0$};
     \node[state] (q_1) [right = 2cm of q_0] {$1$};
     \node[state] (q_2) [right = 2cm of q_1] {$2$};
     \node[state] (q_3) [below = 1.5cm of q_2] {$3$};
     \node[state] (q_4) [left = 2cm of q_3] {$4$};
     \node[state] (q_5) [left = 2cm of q_4] {$5$};
      \path[-]
      (q_0) edge  node {} (q_1)
      (q_0) edge  node {} (q_5)
      (q_1) edge  node {} (q_2)
