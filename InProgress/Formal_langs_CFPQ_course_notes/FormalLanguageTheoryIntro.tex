...
\begin{proof}
  ...
  Алгоритм:\\
  \begin{enumerate}
  \item Найти все цепные пары в грамматике Г.\\
  Найти все цепные пары можно по индукции:\\
  Базис: (A,A) — цепная пара для любого нетерминала, так как A$\derives$А за ноль шагов.\\
  Индукция: Если пара $(A,B_0)$ — цепная, и есть правило $B_0 \to B_1$, то $(A,B_1)$ — цепная пара.
  \item Для каждой цепной пары (A,B) добавить в грамматику Г' все правила вида $A \to a$, где $B \to a$ — нецепное правило из Г.
  \item Удалить все цепные правила
\end{enumerate}
Пусть Г — контекстно-свободная грамматика. Г' — грамматика, полученная в результате применения алгоритма к Г. Тогда L(Г)=L(Г').
\end{proof}
