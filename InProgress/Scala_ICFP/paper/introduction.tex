...
In the section~\ref{sec:CFPQ}  we introduce fromal definition of CFPQ problem and in the 
section~\ref{sec:GLL} we provide basic description of the Meerkat library and SPPF data structure.
Description of our solution is provided in the section~\ref{sec:combinators}.
Then in the section~\ref{sec:examples} we present and discuss some examples: a set of calssical queryes (same generation query, queries to movie dataset~\footnote{The movie database is a traditional dataset for graph databases. Detailed description is available here: \url{https://neo4j.com/developer/movie-database/}. Access date: 16.01.2018}) 
formulated in terms of our library.
We describe results of implemented library evaluation in the section~\ref{sec:evaluation}.
Finally in the section~\ref{sec:conclusion} we summarise our results and discuss possible directions of further research.
