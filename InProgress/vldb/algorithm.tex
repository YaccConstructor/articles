\section{Graph parsing by the calculation of transitive closure}
...
We can, therefore, determine whether $(i,j) \in R_A$ by asking whether $A \in b^+_{i,j}$. Thus, we show how the context-free relations $R_A$ can be calculated by computing the transitive closure $b^+$ of the matrix $b$.
...
\Function{graphParse}{D, G}
    ...
    \State{$n \gets$ number of nodes in $D$}
    \State{$E \gets$ directed edge-relation from $D$}
    \State{$P \gets$ set of production rules in $G$}
    \State{$T \gets$ matrix $n \times n$ in which each element is $\emptyset$}
    \ForAll{$(i,x,j) \in E$}
...
\]
\caption{Input graph for the example query.}
\label{ExampleQueryGraph}
\end{figure}

We provide a step-by-step illustration of the work with the given graph $D$ and grammar $G'$ of the Algorithm~\ref{alg:graphParse}. After matrix initialization in lines \textbf{6-7} of the Algorithm~\ref{alg:graphParse} we have a matrix $T_0$ presented in Figure~\ref{ExampleQueryInitMatrix}.

\begin{figure}[h]
\[
T_0 = \begin{pmatrix}
    \{S_1\} & \{S_3\} & \emptyset \\ \emptyset & \emptyset & \{S_3\} \\ \{S_2\} & \emptyset & \{S_4\}
\end{pmatrix}
\]
...
T_0 \cdot T_0 = \begin{pmatrix}
    \emptyset & \emptyset & \emptyset \\ \emptyset & \emptyset & \{S\} \\ \emptyset & \emptyset & \emptyset
\end{pmatrix}
\]

\[
T_1 = T_0 \cup (T_0 \cdot T_0) = \begin{pmatrix}
    \{S_1\} & \{S_3\} & \emptyset \\ \emptyset & \emptyset & \{S_3, S\} \\ \{S_2\} & \emptyset & \{S_4\}
\end{pmatrix}
\]
