...
In this section we present evalution of our graph querying library.
We measure its performance on classical set of on	tology graphs~\cite{CFGonRDF} : both when the graph is loaded into the RAM and for the integration with the Neo4j database. We also show how may-alias static code analysis can be done by means of the library and compare it with the solution based on the GLL parsing algorithm~\cite{GrigorevR16} and the Trails~\cite{ScalaGraphParsing} library for graph traversals. 

All tests have been perfomed on a computer running Fedora 27 with quad-core Intel Core i7 2.5 GHz CPU and 8 GB of RAM.
...
Querying for ontologies is a well-known graph querying problem~\cite{FndDB}. We evaluate our library on some popular ontoloies which are presented as RDF files in the paper~\cite{CFGonRDF}. 
Frist, we convert RDF files to a labeled directed graph in following manner: we create two edges $(subject,\ predicate,\ object)$ and $(object,\ predicate^{-1},\ subject)$ for every RDF triple $(subject,\ predicate,\ object)$. Then the graph is either loaded into the Neo4j database or loaded directly into the memory. 
...
The performace results are shown in the table~\ref{table:rdfs}. $\#triples$ reflects the size of the RDF file with the corresponding ontology, $\#results$ is a number of pairs of the nodes between which at least one S-path exists. 
...
In this analysis a program is represeted as a Program Expression Graph (PEG)~\cite{Zheng}.
Verticies in a PEG correspond to program expressions and edges are relations between them.
There are two types of edges possible while analysing C source code: \textbf{D}-edge and \textbf{A}-edge.
...
Both Trails and Meerkat support parsing of the graphs located in RAM, so we compare perfomance of Trails and Meerkat on the ontology queries described above: the results are presented in the table~\ref{table:rdfs}.
Trails and Meerkat compute the same results, but Trails is up to 10 times slower than Meerkat.

To summarise, we showed that parser combinators are expressive enough to be used for implemening real queries. 
