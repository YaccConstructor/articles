...
\begin{itemize}
      \item Can nodes A and B be treated as nodes on the same level of hierarchy?
      \item Is there exists path of form $\text{Up}^n \text{Down}^n$?
      \item Show me all path of form $\text{Up}^n \text{Down}^n$ whish starts from node A.
\end{itemize}

\end{minipage}

  \begin{itemize}
    \item (How) Can this automata generates SQL queries?
    \item (How) Can this program can produce some specific chain of calls? 
  \end{itemize}
\end{frame}
