...
\begin{frame}[fragile]
  ...
  \begin{minipage}{0.2\textwidth}
    ...
    \begin{tikzpicture}[overlay,remember picture,auto]
        \draw (8.92, 1.21) node[cross=10pt, color=red] {};
    \end{tikzpicture}
  \end{minipage}
...
\begin{frame}[fragile]
  ...
  \begin{minipage}{0.75\textwidth}    
    \begin{equation*}
      ...
      =      
        ...
        \end{array}\right)
    \end{equation*}
    \mycallout<3-6>[opacity=1]{$(xPos) + (2.9,0.4)$}{У стартовой нет предка}{4.5}
    %\mycallout<2-4>[opacity=1]{$(xPos) + (5.9,1.1)$}{Матрица смежности}{3.5}
    \mycallout<3-6>[opacity=1]{$(xPos) + (10.2,0.4)$}{Стартовая --- предок}{3.5}
    \mycalloutR<3-6>[opacity=1]{$(xPos) + (3.95,0.1)$}{<<Нейтральный элемент>>}{3.2}
  \end{minipage}
  ...
  \begin{minipage}{0.2\textwidth}
    ...
    \pause     
    \begin{tikzpicture}[overlay,remember picture,auto]
        \draw (9.04, 1.21) node[cross=10pt, color=red] {};
    \end{tikzpicture}
  \end{minipage}
  \pause
  \begin{center}
