...
Complete graphs where wor each termial symbol there is edge between two vertices labeled with it.
...
fit f(x) 'perf/2' using 1:4 via a,b,c,d,e

plot 'perf/2' using 1:3 with lines ls 3 ti '$Unamb$',\
     'perf/2' using 1:4 with lines ls 4 ti '$amb$',\
...
set xlabel '$x$-label'
set ylabel '$y$-label
plot 'perf/2' using 1:6 with lines ls 3 ti '$Unamb$',\
     'perf/2' using 1:7 with lines ls 4 ti '$amb$',\
 \end{gnuplot}
\caption{Performance on C graph for grmmars $G_0$ and $G_2$}
\label{pic:DoubleCyclesPerf}
\end{figure}
