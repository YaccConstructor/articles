...
For each descriptor we should examine all outgoing edges which can be bounded with $$\max\limits_{v \in V}\left(deg^+\left(v\right)\right)$$  where $deg^+(v)$ is outdegree of vertex $v$.
