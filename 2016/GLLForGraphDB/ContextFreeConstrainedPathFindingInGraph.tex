...
In this work we are focused on parsing algorithm, and not on the data representation, and de assume thet full input graph can be located in RAM memory by the optimal for our algorithm way.
...
As a result we can define that context-free language constrained path querying meens that we get query as grammar $G$ and result of this query is a set of paths $P=\{p|\Omega(p) \in L(G)\}$.
...
Thus we need $$O\left(|V|^2*\left(\max\limits_{v \in V}\left(deg^+\left(v\right)\right)\right)\right)$$ operation for outgoing edges processing.
...
O\left(V^3*\max\limits_{v \in V}\left(deg^+\left(v\right)\right)\right)
\end{equation}
