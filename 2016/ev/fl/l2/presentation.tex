...
\begin{frame}[fragile]
  ...
  \frametitle{Алгоритм минимизации КА: корректность}   
    \begin{itemize}
      ...
      \item Состояний $A'$ меньше, чем состояний $A_{min} \Rightarrow$ 2 состояниям $A_{min}$ соответствует 1 состояние $A' \Rightarrow$ они эквивалентны. Но по построение $A_{min}$ в нем не может быть эквивалентных состояний. Противоречие
    \end{itemize}
...
\begin{frame}[fragile]
  ...
  \begin{itemize}
    ...
    \end{itemize}
  \item Результат: $\langle \Sigma, Q, q_0 \in Q, T \subset Q, \delta: Q \times \Sigma \rightarrow 2^Q \rangle$
  \begin{itemize}
    \item 
  \end{itemize}
  \end{itemize}
