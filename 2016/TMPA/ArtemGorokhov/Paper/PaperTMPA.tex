...
We only note that most of algorithms are based on classical LL~\cite{ELLParser,AttributedELL} and LR~\cite{ELRParsing} techniques, and they can handle only appropriate subclasses of ECFG.
As a result  do not deal with ambiguities in grammars.
...
Work of the GLL algorithm based on descriptors, it allows to handle all possible derivations.
...
In order to improve performance Elizabeth Scott and Adrian Johnstone offered support of factorized grammars in GLL~\cite{scott2016structuring}. 
The idea is to automatically factorize grammars and use them for parser generation. 

The algorithm creates and queues new descriptors depending on current parse state that we get from dequeued descriptor. 
...
Production $P_0$ factorizes to $P_0'$.
...
by terminal ($\in \Sigma$) or by state ($\in Q$). Further in this paper we will call
