...
We only notice that most of algorithms are based on classical LL~\cite{ELLParser,AttributedELL,PredictiveECFG} and
...
\begin{itemize}
    ...
    \begin{itemize}
        \item children $l_0 \dots l_n (l_i \in \Sigma \cup \Delta(Q))$ iff there exists a
...
For arbitrary grammars, RA can be ambiguous in terms of acceptable paths, and, as a result, it is possible to get multiple derivation trees for one input string.
...
It uses modified function \textbf{getNodeP}, instead of grammar slot it takes a 
state of RA and symbol for new SPPF node: current nonterminal or next RA state.

\input{getNodes.tex}
\input{getNodeP.tex}

Let us demonstrate a SPPF example for ECFG grammar $G_0$~(fig.~\ref{fig:grammarG0}).
...
$C_U$ --- GSS node, $i$ --- position in input string $\omega$, and $C_N$ --- SPPF node. 
