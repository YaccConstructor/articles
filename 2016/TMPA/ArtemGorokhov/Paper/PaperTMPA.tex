...
\item Union all automata for all rhs. Result is FSA. Create a map $M$ from nonterminal to corresponded start state. This map should keep consistent during all fallows steps.
...
\item Replace transitions by nonterminals with transitions labeled by start states by using map $M$. Result of this step is an required RA.
\end{enumerate}
...
So in general this representation of 
