...
So, our goal is to build RA with minimal number of states for given ECFG, which ca be don by following steps.
\begin{enumerate} 
\item For each right-hand side of productions build a FSA using Thompson's method~\cite{Thompson:1968:PTR:363347.363387}.
\item Union all automata for all rhs. Result is FSA. Create a map $M$ from nonterminal to corresponded start state. This map should kept consistent during all fallows steps. 
\item Convert FSA from previous step to deterministic FSA without $\varepsilon$-transitions using the algorithm described in~\cite{aho1974design}.
\item Minimize DFSA by using John Hopcroft's algorithm~\cite{hopcroft1971n}.
\item Replace transitions by nonterminals with transitions labeled by start states by using map $M$. Result of this step is an required RA. Also as a result we define function $\Delta : Q \to N$ where $N$ is nonterminal name.
\end{enumerate}

An example of ECFG to RA transformation is presented in fig.~\ref{fig:fig1}, where state 0 is start state of result RA.
...
Let us to describe modifications of original SPPF construction functions.
...
It uses modified function \textbf{getNodeP} that takes separately nonterminal which currently processed and a state of RA, instead of grammar slot.
