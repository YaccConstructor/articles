...
ECFG is widely used as input format for parser generators, but classical parsing algorithms requires CFG, and as a result, parser generators requires conversion to CFG.
...
In order to provide ability to process grammar in ECFG there is a wide range of parsing techniques and algorithms~\cite{AttributedELL,ELRR,ECFGparsing,ELLParser,ELL,ECFG,ELALR,ELRParsing}.
Detailed overview of results and problems in ECFG processing area is provided in the article ``Towards a Taxonomy for ECFG and RRPG Parsing''~\cite{ECFG}. 
...
As a result, parsing techniques for ECFG is actual problem, and there is no solution for handling arbitrary and ambiguous ECFGs.

The LL-based parsing algorithms is more intuitive than LR-based and can provide better error diagnostic.
...
To summarize, it is possible to simplify language description required for parser generation in case a parser generator is based on generalized algorithm which can handle grammars in ECFG.
Generalized parsing algorithms can handle arbitrary grammars and these demonstrate good performance.
...
In order to provide arbitrary context-free grammars handling, generalized parsing algorithms were purposed.
Generalized LL algorithm~(GLL)~\cite{scott2010gll} inherits properties of classical LL algorithms: it is more intuitive and allows to perform better syntax error diagnostic then generalized LR algorithms.
Also, our experience shows that GLR-based solutions is more complex rather GLL-based, which is in good agreement with the observation in~\cite{ECFG} that LR-based ECFG parsers are very complex. 
Thus, we choose GLL as a base for our solution.
In this section GLL-style parser for arbitrary ECFG processing presented.

\subsection{Generalized LL Parsing Algorithm}%--------------------------------------------------------------------------------------------------------------------------------------------

An idea of the GLL algorithm based on descriptors, it allows to handle all possible derivations.
...
In order to improve performance of GLL Elizabeth Scott and Adrian Johnstone offered support of left-factorized grammars in this parsing algorithm~\cite{scott2016structuring}. 

The basic algorithm creates and queues new descriptors depending on current parse state that we get from dequeued descriptor. 
...
Let us to describe functions which required for SPPF construction.
The function \textbf{getNodeT$(x,i)$}, which create terminal nodes, reused without modification from basic algorithm.
To manipulate other nodes we define function \textbf{getNodes} which can construct two nodes: intermediate and nonterminal (at least one of them, at most both).
...
Fig.~\ref{expPlots1} and fig.~\ref{expPlots2} show experiments results. In general minimized RA version works $33\%$ faster, uses $27\%$ less descriptors, $29\%$ less GSS edges
