...
Parsing is important step of static program analysis because it allows to get structural representation of code which is necessary for analysis.
One of important source of languages' syntax specification is official documentation which often use EBNF to make grammars more compact and expressive.
Moreover, grammar in documentation may be ambiguous.
These facts leads to problems with parsers creation automation by using parser generators, because the most parsing algorithms and tools require transformation to more simple form of grammar (BNF, CNF) and can not handle ambiguous grammars.
...
Also we implement parser generator based on described algorithm and we demonstrate that proposed modifications allow to get parsing performance improvement.
...
Parser generators are often used to automate parser creation: these tools allow to derive parser from grammar.
