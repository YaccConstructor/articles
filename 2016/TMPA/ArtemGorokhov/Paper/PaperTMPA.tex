...
\item There is an outgoing edge $e$ from $C_S$, such that tag of $e = \omega.[C_i]$. 
Create descriptor with ($end of q_1$) and ($j + 1$)
Queue it without checking its existence.
\item There are outgoing edges $e$ from $C_S$, such that tag of $e \in Q$. 
Call \textbf{create} function for each such edge.
...
For input $ aacb $ this parser builds SPPF showed in fig.~\ref{fig:SPPFForG0}.
