...
$F : N \to Q$ --- first states of nonterminals.
\end{mydef}
...
in automaton. But in automaton we have nondeterministic choice because there can be many transitions to other states. 
...
\item Node with label $N_i$ has children $l_0 \dots l_n$ iff exists
path $F(N_i)\cdot \dots \cdot q_m$, $q_m \in P$. 
...
Also we use this automata approach in metagenomic assemblies parsing and it gives visible performance increase.
