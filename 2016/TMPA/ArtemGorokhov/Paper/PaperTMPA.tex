...
Now we can define automaton that well be used for parsing.

\begin{mydef}
Automaton $A$ is a tuple $(\Sigma, N, Q, S, T, \delta)$, where
$\Sigma$ is a set of terminals,
$N$ --- set of nonterminals,
$Q$ --- set of states,
$S \in Q$ --- start state,
$T \in Q$ --- set of accept states,
$\delta : Q \times M \to Q$, $M \in \Sigma \cup N$ --- edges of automata
\end{mydef}
...
Derivation tree of sentence $\alpha$ in the automaton $A=(\Sigma, N, Q, S, T, \delta)$:%grammar $G=(\Sigma, N, S, P)$:

\begin{itemize}
\item Ordered rooted tree. Root labeled with $S$
\item Leafs are terminals $\in \Sigma$
\item Nodes are nonterminals
\item Node with label $N_i$ has children $l_0 \dots l_n$ iff for $\omega = l_0 \cdot l_1 \dots\ l_n\in (\Sigma \cup N)^*$ exists $p \rightarrow M \in P$ such that $\omega \in L(M)$
\end{itemize}
