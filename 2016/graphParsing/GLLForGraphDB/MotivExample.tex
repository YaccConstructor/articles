...
This language is a subset of balanced brackets language and in practice may be used to describe many different relations: a n-th generation in parent-child, a correct order of an open-close operations for resources, etc.

Let's a graph $M=(\{0;1;2;3\},E,\{A;B\})$ is presented in figure~\ref{input}. Labels on edges represent one of mentioned relations.
...
The result of presented query for given graph is an infinite set of paths, hence it cannot be constructed explicitly. 
Moreover, for some tasks it can be necessary to get the structure of result with respect to given grammar.
Late we show how to get finite representation of query result structure in terms of derivation in grammar.
