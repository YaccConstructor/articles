...
\begin{enumerate}
\item $n \rightarrow \alpha \cdot x \beta $ when $x$ is a terminal and $x = input[i]$. In this case we should to move both pointers to right: $i = i + 1$, new slot $= n \rightarrow \alpha  x \cdot \beta $.
\item $n \rightarrow \alpha \cdot x \beta $ when $x$ is nonterminal. In this case we should to save return address $n \rightarrow \alpha x \cdot \beta $ in stack and move pointer in grammar to position $x \rightarrow \cdot \gamma$.
